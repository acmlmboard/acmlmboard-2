 ...
 * -> Add beastie-specific title text.
 ...
 *    /acmlmon.php?catch=[NUMBER HERE] and get them all.
 * 
 */
...
$monRequest = mysql_query("SELECT * FROM acmlmon WHERE id <= ".$roll);
$monData = array();
...
$monMarkup = "<img id=\"acmlmon\" style=\"opacity: 0.75; -moz-opacity: 0.75; position: fixed; ".$x."; ".$y."; z-index: 999\" src=\"img/acmlmon/".$pic."\" title=\"".$monData['title']."\" onclick=\"capture()\" />";

$monScript = "<script language=\"javascript\">
	function capture()
	{
		document.getElementById(\"acmlmon\").style['display'] = \"none\";
		...
		x.open('POST', 'acmlmon.php?catch=".$monData['id']."', true);
		x.send(null);
	}
</script>";

...
";
  ...
  && $thread=$sql->fetchq("SELECT p.id,t.title ttitle,f.title ftitle,t.forum,f.minpower "
                         ...
                         ."WHERE p.date=$user[lastpost] AND p.user=$uid"))
...
".      "        $L[TD2]>
".      "          ".sprintf('%d:%02d',$user[tzoff]/3600,abs($user[tzoff]/60)%60)." from server,
".      "          ".sprintf('%d:%02d',($user[tzoff]-$loguser[tzoff])/3600,abs(($user[tzoff]-$loguser[tzoff])/60)%60)." from you
".      "          <br>(current time: ".date($dateformat,ctime()+$user[tzoff]).")
".      "      $L[TR]>

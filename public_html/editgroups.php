...
if (isset($_GET['deletedgroups'])) {
	...
	while ($deletedgroup = $sql->fetch_assoc($deletedgroups)) {
		...
		if (!$deletedgroup['visible'])
			$name = "<span style=\"opacity: 0.6;\">{$name}</span>";
		...
		$actions = array();
		if ($caneditperms)
			$actions[] = array('href' => 'editgroups.php?deletedgroups&act=undelete&id=' . urlencode(packsafenumeric($deletedgroup['id'])), 'title' => 'Undelete',
...
elseif ($act == 'new' || $act == 'edit') {
	...
	while ($group = $sql->fetch_assoc($groups)) {
		...
		$actions = array();
		if ($caneditperms)
			$actions[] = array('href' => 'editperms.php?gid=' . $group['id'], 'title' => 'Edit permissions');
		$actions[] = array('href' => 'editgroups.php?act=edit&id=' . $group['id'], 'title' => 'Edit');
		if ($caneditperms)
			$actions[] = array('href' => 'editgroups.php?act=delete&id=' . urlencode(packsafenumeric($group['id'])), 'title' => 'Delete',

...
//Various variables.
$past = isset($_GET['past']) ? (int) $_GET['past'] : 0;
...
if ($year < -1000000000000000000 || $year > 1000000000000000000)
...
if ($dpur) {
	...
	foreach ($dpur as $usr => $pnts) {
		if ($pnts < $t)
			$r = $q;
		$mpur = $sql->fetch_assoc($sql->query("SELECT " . userfields() . " FROM users WHERE id=$usr"));

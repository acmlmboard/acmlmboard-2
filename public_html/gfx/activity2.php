...
for($i=0;$i<320;$i+=20) {
	...
	imagettftext($im,7,0, 1034, $i+14, $cwhite, getcwd()."/verdana.ttf", (310-$i)/5);
...
imagettftext($im,7,0,7,30,$cwhite, getcwd()."/verdana.ttf", "Activity stats for $n\n  bold: 8-day average\n  thin: daily postcount");
...
do{
  ...
  while($q+1 != $day['q']) {
    ...
    if(!(($x-2)%64)) {
      imagettftext($im,7,0, $x, 10, $cwhite, getcwd()."/verdana.ttf", $day['date']);
    }
    $yold=$abuf->get();
  }
  $q=$day['q'];
  ...
  $users=checkvar('day','users');
  $posts=checkvar('day','posts');
  $threads=checkvar('day','threads');
  $views=checkvar('day','views');
  $yold=$abuf->get();

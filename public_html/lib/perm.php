...
function can_view_cat($cat) {
	...
	if (!has_perm('view-public-categories')) {
		return false;
	}
	...
	if ($cat['private']) {
		...
		if (!has_perm('view-all-private-categories') && !has_perm_with_bindvalue('view-private-category', $cat['id'])) {
			return false;
		}
	}
	return true;
...
function can_edit_post($post) {
	global $loguser;
	if ($post['user'] == $loguser['id'] && has_perm('update-own-post')) {
		return true;
	} else if (has_perm('update-post')) {
		return true;
	} else if (can_edit_forum_posts($post['tforum'])) {
		return true;
	}
...
function can_edit_group_assets($gid) {
	global $sql, $loguser;
	if (has_perm('edit-all-group')) {
		return true;
	} else if (has_perm_with_bindvalue('can-edit-group', $gid)) {
		return true;
	}
	return false;
}
...
function can_edit_user_assets($gid) {
	global $sql, $loguser;
	if (has_perm('edit-all-group-member')) {
		return true;
	} else if (has_perm_with_bindvalue('can-edit-group-member', $gid)) {
		return true;
	}
	return false;
}
...
function can_edit_user($uid) {
	...
	if (is_root_gid($gid) && !has_perm('no-restrictions')) {
		return false;
	}
	if ((!can_edit_user_assets($gid) && $uid != $loguser['id']) && !has_perm('no-restrictions')) {
		return false;
	}

	if ($uid == $loguser['id'] && has_perm('update-own-profile')) {
		return true;
	} else if (has_perm('update-profiles')) {
		return true;
	} else if (has_perm_with_bindvalue('update-user-profile', $uid)) {
		return true;
	}
	return false;
}
...
function can_edit_user_moods($uid) {
	global $sql, $loguser;
	if ($uid == $loguser['id'] && has_perm('update-own-moods'))
		return true;
	else if (has_perm('edit-moods'))
		return true;
	else if (has_perm_with_bindvalue('update-user-moods', $uid))
		return true;
	return false;
}
...
function can_view_forum($forum) {
	...
	if (!can_view_cat(array('id' => $forum['cat'], 'private' => $forum['cprivate'])))
		return false;

	//can view public forums
	//HOSTILE DEBUGGING echo 'checking for public forum<br>';
	if (!has_perm('view-public-forums'))
		return false;
	...
	if ($forum['private']) {
		...
		if (!has_perm('view-all-private-forums') &&
				!has_perm_with_bindvalue('view-private-forum', $forum['id']))
			return false;
	}
	return true;
}
...
function can_create_forum_thread($forum) {
	...
	global $log;
	if ($forum['readonly'] && !has_perm('override-readonly-forums'))
		return false;

	//must fulfill the following criteria
	//can create public threads
	//HOSTILE DEBUGGING echo 'checking for public forum<br>';
	if (!has_perm('create-public-thread'))
		return false;
	if (!has_perm('post-offline') && !$log)
		return false;
	...
	if ($forum['private']) {
		...
		if (!has_perm('create-all-private-forum-threads') &&
				!has_perm_with_bindvalue('create-private-forum-thread', $forum['id']))
			return false;
	}
	return true;
}
...
function can_create_forum_post($forum) {
	...
	//can create public threads
	//HOSTILE DEBUGGING echo 'checking for public forum<br>';
	if (!has_perm('create-public-post'))
		return false;
	if (!has_perm('post-offline') && !$log)
		return false;
	...
	if ($forum['private']) {
		...
		if (!has_perm('create-all-private-forum-posts') &&
				!has_perm_with_bindvalue('create-private-forum-post', $forum['id']))
			return false;
	}
	return true;
}
...
function can_create_forum_announcements($forumid) {
	if (!has_perm('create-all-forums-announcement') &&
			!has_perm_with_bindvalue('create-forum-announcement', $forumid))
		return false;
...
function can_create_locked_posts($forumid, $threadid) {
	...
	if (has_perm_with_bindvalue('override-closed-forum', $forumid))
		return true;
	if (has_perm_with_bindvalue('override-closed-thread', $threadid))
		return true;
...
function can_post_consecutively($forumid) {
	if (has_perm('consecutive-posts'))
		return true;

	if (has_perm_with_bindvalue('create-consecutive-forum-post', $forumid))
		return true;
...
function can_edit_forum_posts($forumid) {
	if (!has_perm('update-post') &&
			!has_perm_with_bindvalue('edit-forum-post', $forumid))
		return false;
...
function can_delete_forum_posts($forumid) {
	if (!has_perm('delete-post') &&
			!has_perm_with_bindvalue('delete-forum-post', $forumid))
		return false;
...
function can_edit_forum_threads($forumid) {
	if (!has_perm('update-thread') &&
			!has_perm_with_bindvalue('edit-forum-thread', $forumid))
		return false;
...
function can_view_forum_post_history($forumid) {
	if (!has_perm('view-post-history') &&
			!has_perm_with_bindvalue('view-forum-post-history', $forumid))
		return false;
	return true;
}
...
function can_delete_forum_threads($forumid) {
	if (!has_perm('delete-thread') &&
			!has_perm_with_bindvalue('delete-forum-thread', $forumid))
		return false;

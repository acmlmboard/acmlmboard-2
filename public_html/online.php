...
$users = $sql->query("SELECT * FROM users WHERE lastview>" . (ctime() - $time) . " $hiddencheck ORDER BY lastview DESC");
...
for ($i = 1; $guest = $sql->fetch_assoc($guests); $i++) {
	...
	print "<tr class=\$tr\" align=\"center\">
" . "    <td class=\"b n1\">$i.</td>
" . "    <td class=\"b\" align=\"left\"><span title=\"" . htmlspecialchars($guest['useragent']) . "\" style=white-space:nowrap>" . htmlspecialchars($guest['useragent']) . "</span></td>

...
if ($targetuserid == 0 || !empty($act) && ($act != 'Save and continue' || $act != 'Save and finish')) {
...
if ($act == 'Save and continue' || $act == 'Save and finish') {
	...
	if ($radar_add != -1) {
		...
		$qr = 'INSERT INTO `post_radar` ( `user_id` , `user2_id` , `ctime` ) VALUES (' . $targetuserid . ', ' . $radar_add . ', UNIX_TIMESTAMP( ))';
		$sql->query($qr);
	}
	if ($radar_rem != -1) {
...
if (!$act || $act == 'Save and continue') {
	...
	print "<form action=\"postradar.php\" method=\"post\" enctype=\"multipart/form-data\" /><table cellspacing=\"0\" class=\"c1\">" . catheader('Edit Post Radar');
	...
	foreach ($ulist as $z => $k)
		$uchoices[$k['id']] = $z . ' -- ' . $k['posts'] . ' posts';
	$radar_remlist = array();

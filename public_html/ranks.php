...
if (!is_numeric($getrankset))
	$getrankset = 1; //Double checking.. 
$totalranks = $sql->query_result("SELECT count(*) FROM `ranksets` WHERE id > '0';");
...
while ($row = $sql->fetch_assoc($allusers)) {
...
if (($request_action == 'addrankset' || $request_action == 'editrankset' || $request_action == 'deleterankset' || $request_action == 'editranks') && !has_perm('edit-ranks')) {
	error("Error", "You have no permissions to do this!<br> <a href=./>Back to main</a>");
}

if ($request_action == 'deleterankset' && ($getrankset < 2 || $getrankset > $totalranks)) {
	error("Error", "The Mario, Dots, and None ranksets may not be deleted on the board.<br> <a href=./>Back to main</a>");
}

if ($request_action == 'deleterankset' && $getrankset >= 2 && $getrankset != unpacksafenumeric($_GET['token'])) {
	error("Error", "Invalid token.<br> <a href=./>Back to main</a>");
}

if (($request_action == 'addrankset' && $request_action == 'Submit' && $_POST['newname'] == '') || ($request_action == 'editrankset' && $request_action == 'Submit' && $_POST['editname'] == '')) {
	error("Error", "Please enter a name for this rankset.<br> <a href=./>Back to main</a>");
}

if ($request_action == 'addrankset' && $request_action == 'Submit' && has_perm('edit-ranks')) {
	$newname = $sql->escape($_POST['newname']);
	$getrankset = $sql->query_result("SELECT MAX(id) FROM ranksets");
	if (!$getrankset)
		$getrankset = 0;
	$getrankset++;
	$sql->prepare_query("INSERT INTO ranksets (`id`,`name`) VALUES (?,?)", array($getrankset, $newname));
	redirect("ranks.php", 1);
}

if ($request_action == 'editrankset' && $request_action == 'Submit' && has_perm('edit-ranks')) {
	$getrankset = intval($getrankset);
	$editname = $sql->escape($_POST['editname']);
	$sql->prepare_query("UPDATE ranksets SET `name`=?  WHERE id=?", array($editname, $getrankset));
	redirect("ranks.php", 2);
}

if ($request_action == 'deleterankset' && $getrankset >= 2 && $getrankset == unpacksafenumeric($_GET['token']) && has_perm('edit-ranks')) {
	$getrankset = intval($getrankset);
	$sql->prepare_query("DELETE FROM ranksets WHERE id=?", array($getrankset));
...
while ($rank = $sql->fetch_assoc($allranks)) {
...
if ($request_action == 'addrankset' && has_perm('"edit-ranks')) {
...
if ($_GET[action] == 'editrankset' && has_perm('"edit-ranks')) {
	pageheader("Rankset Listing");
	$editrankset = $sql->query_result("SELECT `name` FROM `ranksets` WHERE `id`='$getrankset'");
...
while ($rank = $sql->fetch_assoc($ranks)) {
	$neededposts = $rank['p'];

<?php
  ...
  if($act=='join'){
    ...
    if($room && $room[active]==0 && $room[users]<$room[usermax] && (!$room[lvmin] || $room[lvmin]<=$logst[lvl]) && (!$room[lvmax] || $room[lvmax]>=$logst[lvl])){
      $numA=$sql->query_result("SELECT count(*) FROM usersrpg WHERE room=$id AND side=0");
      $numB=$sql->query_result("SELECT count(*) FROM usersrpg WHERE room=$id AND side=1");
      $side=($numA<=$numB?0:1);
  ...
  if($act=='exit'){
    $id=$sql->query_result("SELECT room FROM usersrpg WHERE id=$uid");
    $sql->query("UPDATE usersrpg SET room=0 WHERE id=$uid");
    $sql->query("UPDATE rpgrooms SET users=users-1 WHERE id=$id");

    $num=$sql->query_result("SELECT users FROM rpgrooms WHERE id=$id");
    if(!$num){
  ...
  if($act=='ready'){
    ...
    if($x){
      $a=$sql->query_result("SELECT count(*) FROM usersrpg WHERE room=$loguser[room] AND ready=0");
      $s=$sql->query_result("SELECT count(DISTINCT side) FROM usersrpg WHERE room=$loguser[room]");

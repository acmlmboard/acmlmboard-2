...
$copyright = htmlentities("Posts are owned by the poster. Acmlmboard $abversion software Copyright 2005-2015 $boardprog");
$generator = htmlentities("Acmlmboard $abversion ($abdate)");
...
if ($thread > 0) {
	...
	$posts = $sql->query("SELECT $fieldlist pt.*, p.*, t.lastdate, t.title ttitle, f.id fid, f.title ftitle "
			. "FROM posts p "
...
" . "      <description>Post by &lt;a href=\"$config[base]$config[path]profile.php?id=$p[u2id]\"&gt;$p[u2name]&lt;/a&gt;, " .
				...
				"in &lt;a href=\"$config[base]$config[path]forum.php?id=$p[fid]\"&gt;$p[ftitle]&lt;/a&gt;</description>

...
if ($action == "save" && has_perm('manage-shop-items')) {
	$set = "";
	$stype = "";
	if ($id != -1) {
		for ($i = 0; $i < 9; $i++) {
...
if (!has_perm('use-item-shop')) {
	...
	switch ($action) {
		case 'delete': //Added (Sukasa)
			if ($id > 0) { //Can't delete nothing
				$sql->query("DELETE FROM items WHERE id='$id'");
				for ($i = 1; $i < 7; $i++) {
					$sql->query("UPDATE usersrpg SET `eq$i` = 0 WHERE `eq$i`='$id'");
				}
			}

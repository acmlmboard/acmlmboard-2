...
0220  blackhole89     readded check for forum minpower; this appears
  ...
  ****************************************************/
  ...
  if($_GET[pin] && $_GET[rev] && ismod(getforumbythread($tid))) {
...
";
    ...
    if($loguser[power]<$thread[minpower]){
      pageheader("(Restricted Thread)",0);
      print 
          "$L[TBL1]>
".        "  $L[TR2]>
".        "    $L[TD1c]>
".        "      You may not access a thread in a forum restricted to users with a powerlevel higher than yours.
".        "$L[TBLend]
";
      pagefooter();
      die();
...
";
  ...
  $modlinks='<br>';
  if($tid && (ismod($thread[forum]) || ($loguser[id] == $thread[user] && !$thread[closed] && $loguser[renamethread]))){
    $link="<a href=javascript:submitmod";
    if (ismod($thread[forum])) {
      if($thread[sticky])
...
";
  while($post=$sql->fetch($posts)){
    ...
    if(ismod($pthread[forum]) && $post[id]==$_GET[pin]) $post[deleted]=false;
    print "<br>
